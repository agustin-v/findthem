...
// Mirrors apps/ui/src/lib/zones.ts — keep the coordinator and volunteer
// apps visually consistent. Backend source of truth for the status enum is
// apps/api/lib/findthem_api/searches/zone.ex.
...
export function zonesToGeoJSON(zones: Zone[]): FeatureCollection<Polygon> {
  ...
  for (const zone of zones) {
    ...
    // malformed index, and this runs synchronously in the map screen's
    // render path with no ErrorBoundary. A skipped zone is far better than
    // a crashed map for every volunteer viewing the same search.
