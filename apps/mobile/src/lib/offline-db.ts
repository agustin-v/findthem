...
async function migrate(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  // Single migration so far — a future schema change (e.g. Story #54's
  // outbox table) adds a new `if (version < N)` step here rather than
  // rewriting this one, so an app update never re-runs a step a device
  // already applied.
  //
  ...
  // plaintext next to an encrypted database that protects nothing but its
  // own metadata about that file — caught by adversarial review before
  // this shipped. SQLite handles a multi-MB TEXT column fine, and a single
  // INSERT/UPDATE is already atomic (SQLite's own crash safety), so the
  // temp-write-then-rename dance a real file would have needed isn't
